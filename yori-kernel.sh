...
clone() {
    ...
    git clone https://github.com/arefinx/kernel_xiaomi_sunny $KERNEL_DIR 
    git clone https://github.com/arefinx/AnyKernel3.git $AK_DIR
    ...
    git clone https://github.com/LineageOS/android_prebuilts_clang_kernel_linux-x86_clang-r416183b $CLANG_DIR
}
...
build_end() {
    ...
    python2 $DIR/scripts/ufdt/libufdt/utils/src/mkdtboimg.py  create $KERNEL_DIR/out/arch/arm64/boot/dtbo.img --page_size=4096 $KERNEL_DIR/out/arch/arm64/boot/dts/vendor/qcom/sunny-sm8250-overlay.dtbo
...
rm -rf $AK_DIR/Image.gz-dtb $AK_DIR/Moonlight*.zip  $AK_DIR/dtbo.img
mkdir -p $KERNEL_DIR/out
make O=out clean

# Build device 1
build_setup
sunny_defconfig $DEFCONFIG
build_kernel
build_end $DEVICE
